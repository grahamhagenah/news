...
- GitHub Actions rebuilds and deploys to GitHub Pages on every push and every 15 minutes (started by a cron-job.org job calling the workflow_dispatch API, since GitHub's own schedule skips most runs), and makes an empty commit if the repo goes 50 days without one, so GitHub doesn't switch the schedule off.
