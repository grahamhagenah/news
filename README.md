...
- Its icon is a red pushpin, [Tabler Icons](https://tabler.io/icons)' "pin" (filled, MIT license), in `events/pushpin/`: `favicon.svg`, and `apple-touch-icon.png` for a phone's home screen, made by `share_cards.py`. In the header, the same pin as a line drawing (Tabler's outline "pin") comes before the name, and the city's name is lighter than Pushpin's.
