...
- Each listing has a share button at the right end of its row (on a screen with a pointer, only while the row is hovered): the device's share sheet where there is one, with "Aldous Harding · The Sinclair · Mon, Sep 14, 8pm" and a link to just it on Pushpin Boston (`?venue=…&q=…`); elsewhere the same, copied. Shares are counted in GoatCounter as `share/<venue>`.
